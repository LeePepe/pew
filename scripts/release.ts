#!/usr/bin/env bun
/**
 * Automated release script for pew monorepo.
 *
...
// ---------------------------------------------------------------------------
// Monorepo version targets (from bump-version.ts)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Monorepo version update (replaces bump-version.ts)
// ---------------------------------------------------------------------------
...
function updateVersionInFile(
  ...
  if (target.pattern === "json-version") {
    ...
    if (!re.test(content)) {
      console.error(
        `  ✗ ${target.path} — version "${oldVersion}" not found`,
      );
      return false;
    }
    // Re-create regex since .test() consumes lastIndex
...
/**
 * Format a CHANGELOG section matching pew's existing style:
 *   ## v1.14.5
 *   ### Added
 *   - ...
 */
...
async function main(): Promise<void> {
  ...
  console.log("📦 pew release");
  console.log(`   Current version: ${currentVersion}`);
  console.log(`   New version:     ${newVersion}`);
  console.log(`   Bump type:       ${bumpArg}`);
  console.log(`   Branch:          ${branch}`);
  console.log(`   Last tag:        ${lastTag ?? "(none)"}`);
  console.log(`   Targets:         ${VERSION_TARGETS.length} files`);
  console.log("");

  // --- Phase 1: Version bump across monorepo ---
  console.log(
    `📝 Phase 1: Updating version in ${VERSION_TARGETS.length} files...\n`,
  );

  if (isDryRun) {
    console.log(
      `   [dry-run] Would update ${VERSION_TARGETS.length} files: ${currentVersion} → ${newVersion}`,
    );
  ...
  // Stage all modified version files + lockfile + changelog
  const filesToStage = [
