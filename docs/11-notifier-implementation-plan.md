...
2. **信号文件 (`notify.signal`)**：写入递增计数器（非 mtime），每次 notify 调用 +1
3. **dirty follow-up**：sync 开始前读 signal 值作为 baseline；sync 完成后再读，若值 !== baseline 则补跑

> **为什么不用 mtime**：某些文件系统（HFS+、FAT32）mtime 精度只到秒，同秒内的 notify 会因 `mtime === startTime` 被漏掉。用文件内容中的递增计数器完全消除精度问题。
 ...
 * Returns null when the lock is already held (another sync is running).
 */
...
notify.cjs (hook callback):
  1. 原子递增信号文件 notify.signal 的计数器（每次 +1）
...
pew notify --source=xxx:
  1. 尝试 flock(sync.lock, LOCK_EX | LOCK_NB)
     ├── 成功 → 持有锁，继续
     └── 失败 (EAGAIN) → 递增 notify.signal 确保 dirty，exit 0
  2. 读取 notify.signal 值作为 baseline
  3. 执行 sync
  4. 再读 notify.signal，值 !== baseline？
...
pew notify 进程 A                    pew notify 进程 B
  ...
  sync 完成
     │
     ▼
  read(signal) → "6" !== baseline "5"？
     │
    是（进程 B 递增过 signal）
     │
     ▼
  dirty follow-up：baseline = "6"，再跑一轮 sync
     │
     ▼
  read(signal) → "6" === baseline "6"？
     │
    是
     │
     ▼
...
| sync 运行中，新 notify 到达 | 新进程获取锁失败 → 递增 signal → exit 0 |
| sync 完成，signal 值变化 | dirty follow-up，再跑一轮 |
| sync 完成，signal 值不变 | 释放锁，正常退出 |
| sync 运行中，N 个 notify 到达 | 所有都递增 signal + exit 0，但只触发 1 轮 follow-up |
...
try {
  ...
  if (err.code === "EAGAIN" || err.code === "EWOULDBLOCK") {
    // 另一个进程持有锁 — 递增 signal 确保 dirty，退出
    await incrementSignal(stateDir, fs);
    await fd.close();
...
/** Read the current signal counter value. Returns "0" if file doesn't exist. */
async function readSignal(stateDir: string, fs: FsOps): Promise<string>;

/**
 * Atomically increment the signal counter.
 * Reads current value, parses as integer, writes value + 1.
 * Non-numeric or missing file → starts from 1.
 */
async function incrementSignal(stateDir: string, fs: FsOps): Promise<void>;
```
...
| 2 | 获取锁失败（mock EAGAIN）| 返回 null（Promise\<CoordinatorRunResult \| null\>），executeSyncFn 不被调用 |
| 3 | sync 完成后 signal 计数器变化 | executeSyncFn 被调用 2 次（原始 + follow-up），result.hadFollowUp = true |
| 4 | sync 完成后 signal 计数器不变 | executeSyncFn 只调用 1 次 |
| 5 | follow-up 运行中 signal 又递增 | executeSyncFn 调用 3 次（最多 maxFollowUps） |
...
| 9 | signal 文件不存在 | baseline 为 "0"，不 follow-up |
| 10 | runId 格式正确 | ISO 时间戳 + 随机后缀 |
| 11 | signal 内容非数字 | 视为 "0"，正常递增到 "1" |
...
2. **递增信号文件** `~/.config/pew/notify.signal` 中的计数器（用于 Coordinator dirty 检测，不依赖 mtime）
3. detached spawn `<PEW_BIN> notify --source=<source>`
4. **对 Codex**：链式调用原始 notify（读取 `~/.config/pew/codex_notify_original.json`）
5. 始终 exit 0

**注意**：notify.cjs **不做 throttle**。节流完全由 Coordinator 的文件锁控制 — 如果有 sync 在跑，新的 `pew notify` 进程递增 signal 后立即退出；如果没有 sync 在跑，立即启动。这避免了 v1 设计中"throttle 窗口内丢事件"的问题。
...
// 2. 递增信号文件计数器（每次 +1，不依赖 mtime）
try {
  mkdirSync(STATE_DIR, { recursive: true });
  const signalPath = join(STATE_DIR, "notify.signal");
  let counter = 0;
  try { counter = parseInt(readFileSync(signalPath, "utf8"), 10) || 0; } catch (_) {}
  writeFileSync(signalPath, String(counter + 1));
} catch (_) {}
...
- **dirty follow-up**：`notify.signal` 文件计数器值比较（非 mtime）
