...
3. **dirty follow-up**：sync 开始前读 signal 文件大小作为 baseline；sync 完成后再读大小，若 !== baseline 则补跑；无 dirty 时截断文件
...
> - Coordinator 持锁期间在无 dirty 退出前截断 signal 文件，防止无限增长
...
pew notify --source=xxx:
  1. 尝试 flock(sync.lock, LOCK_EX | LOCK_NB)
     ├── 成功 → 持有锁，继续
     └── 失败 (EAGAIN) → append "\n" 到 notify.signal 确保 dirty，exit 0
  2. baseline = stat(notify.signal).size（文件不存在则 0）
  3. 执行 sync
  4. 再 stat(notify.signal).size，size !== baseline？
     ├── 是 → dirty follow-up：回到步骤 2（最多 maxFollowUps 轮）
     └── 否 → 截断 signal 文件为空，释放锁，exit 0
```
...
pew notify 进程 A                    pew notify 进程 B
  ...
  sync 完成
     │
     ▼
  stat(signal).size → 6 !== baseline 5？
     │
    是（进程 B append 过 signal）
     │
     ▼
  dirty follow-up：baseline = 6，再跑一轮 sync
     │
     ▼
  stat(signal).size → 6 === baseline 6？
     │
    是
     │
     ▼
...
| sync 运行中，新 notify 到达 | 新进程获取锁失败 → append signal → exit 0 |
| sync 完成，signal size 变化 | dirty follow-up，再跑一轮 |
| sync 完成，signal size 不变 | 截断 signal，释放锁，正常退出 |
| sync 运行中，N 个 notify 到达 | 所有都 append signal + exit 0，但只触发 1 轮 follow-up |
...
try {
  ...
  if (err.code === "EAGAIN" || err.code === "EWOULDBLOCK") {
    // 另一个进程持有锁 — append signal 确保 dirty，退出
...
/**
 * Read the current signal size (number of appended bytes).
 ...
 * Called by the lock holder after a clean exit (no dirty follow-up).
 */
...
| 2 | 获取锁失败（mock EAGAIN）| 返回 null，executeSyncFn 不被调用，signal 被 append |
| 3 | sync 完成后 signal size 变化 | executeSyncFn 被调用 2 次（原始 + follow-up），result.hadFollowUp = true |
| 4 | sync 完成后 signal size 不变 | executeSyncFn 只调用 1 次，signal 被截断 |
| 5 | follow-up 运行中 signal 又有 append | executeSyncFn 调用 3 次（最多 maxFollowUps） |
...
| 11 | 无 dirty 退出后 signal 文件被截断为 0 bytes | stat(signal).size === 0 |
...
**注意**：notify.cjs **不做 throttle**。节流完全由 Coordinator 的文件锁控制 — 如果有 sync 在跑，新的 `pew notify` 进程 append signal 后立即退出；如果没有 sync 在跑，立即启动。这避免了 v1 设计中"throttle 窗口内丢事件"的问题。
...
- **dirty follow-up**：`notify.signal` 文件 size 比较（O_APPEND 原子追加，非 mtime 或计数器）
