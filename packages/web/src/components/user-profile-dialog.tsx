...
function DialogSkeleton({ name, image }: { name?: string | null; image?: string | null }) {
    ...
    <div className="space-y-4">
      ...
      <StatGrid columns={4}>
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="rounded-[var(--radius-card)] bg-secondary p-3 space-y-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_200px] gap-3">
        ...
        <div className="rounded-[var(--radius-card)] bg-secondary p-3 flex items-center justify-center">
  ...
  return (
    <>
      ...
      {loading && !data && !error && !notFound && (
        <DialogSkeleton name={name ?? null} image={image ?? null} />
      )}
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-2xl max-h-[90vh] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border bg-card p-5 shadow-lg data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95">
